...
QString _readExifData(const QString &imagePath) {
...
QString readExifData(const QString &imagePath) {
    ...
    exif.loadFromJpeg(&file);

    QString output;
    if (exif.isValid()) {
        output += "Camera Make: " + exif.value(QExifImageHeader::CameraManufacturer).toString() + "\n";
        output += "Camera Model: " + exif.value(QExifImageHeader::CameraModel).toString() + "\n";
        output += "Date Taken: " + exif.value(QExifImageHeader::DateTimeOriginal).toDateTime().toString() + "\n";
        output += "ISO: " + QString::number(exif.value(QExifImageHeader::ISOSpeedRatings).toInt()) + "\n";
    } else {
